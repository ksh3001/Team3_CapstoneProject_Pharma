# task-018 — Implement mode controller and AI-disabled path (FR-006)
...
| Depends on | 010,001 |
| FR / area | FR-006 |
| Maps AC / NFR | AC-050, AC-051; NFR-01 |
| Prompt | prompts/10_implementation_tasks.md |

## 1. Goal

Implement runtime mode (deterministic_offline / ai_disabled) with no-op LLM port and narrator refusal.
...
Enabling LLM in assessed default; hanging on model down.
...
- [ ] Implement runtime/mode.py and ports/llm.py no-op
- [ ] mode_violation behavior when AI path requested while disabled
- [ ] Make task-010 tests pass

## 5. Acceptance checks

Proves: **AC-050, AC-051; NFR-01** (see submission/specs/features/acceptance_criteria_register.md and submission/specs/testing/ac_test_plan.md).
